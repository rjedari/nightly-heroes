...
<div class="absolute bottom-0 left-1/2 transform -translate-x-1/2">    
     <div class="overflow-y-hidden overflow-x-auto flex no-scrollbar max-w-36">
       <div v-for="item in list" @click.prevent="slider(item.id)" :key="item.id" 
       :class="{'bg-white opacity-100': item.id === activeId, 
      'bg-gray-900 opacity-100': item.id !== activeId,
      'active-dot' :item.id === activeId
    }"
      class="w-2.5 h-2.5 m-3 cursor-pointer border-gray-400 border bg-gray-700 opacity-50 rounded-full"> <div class="w-2.5 h-2.5"></div>
    </div>
  </div>
    </div>
  </div>
